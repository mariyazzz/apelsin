...
var calendarHeatmap = {
     ...
     * Initialize
     */
    init: function(parentId ,data, color, overview, handler) {

        // Set calendar data
        calendarHeatmap.data = data;

        calendarHeatmap.parentId = parentId,
        ...
        calendarHeatmap.div_x= document.getElementsByClassName('calendar-heatmap')[0].offsetLeft;
        calendarHeatmap.div_y= document.getElementsByClassName('calendar-heatmap')[0].offsetTop;
    ...
    createElements: function() {
        ...
        document.getElementById(calendarHeatmap.parentId).appendChild(container);

        // Create svg element
        var svg = d3.select(container).append('svg')
            .attr('class', 'svg');
        calendarHeatmap.svg = svg;
        ...
        var item_block = calendarHeatmap.items.selectAll('.item-block-year')
            ...
            .on('mouseover', function(d) {
                ...
                calendarHeatmap.tooltip.html(tooltip_html)
                    .style('left', div_x + x + 'px')
                    .style('top', div_y + y + 'px')
                    .transition()
    ...
    drawYearOverview: function() {
        ...
        calendarHeatmap.items.selectAll('.item-circle')
            ...
            .on('mouseover', function(d) {
                ...
                // Show tooltip
                calendarHeatmap.tooltip.html(tooltip_html)
                    .style('left', calendarHeatmap.div_x + x + 'px')
                    .style('top', calendarHeatmap.div_y + y + 'px')
                    .transition()
                    .duration(calendarHeatmap.settings.transition_duration / 2)
                    .ease(d3.easeLinear)
                    .style('opacity', 1);
    ...
    drawDayOverview: function() {
        ...
        calendarHeatmap.settings.height = calendarHeatmap.settings.label_padding + project_labels.length * (calendarHeatmap.settings.item_size + calendarHeatmap.settings.gutter);
        calendarHeatmap.svg.attr('width', calendarHeatmap.settings.width).attr('height', calendarHeatmap.settings.height);
        ...
        calendarHeatmap.items.selectAll('.item-block')
            ...
            .on('mouseout', function() {
                if (calendarHeatmap.in_transition) { return; }
                calendarHeatmap.hideTooltip();
                calendarHeatmap.items.selectAll('.day-line')
                    .transition()
                    .duration(calendarHeatmap.settings.transition_duration)
                    .ease(d3.easeLinear)
                    .style('opacity', 0);
        ...
        calendarHeatmap.items.selectAll('.day-line').remove();
        calendarHeatmap.labels.selectAll('.label-time')
            ...
            .attr('x', function(d, i) {
                calendarHeatmap.items.append('line')
                    .attr('x1', timeScale(i))
                    .attr('y1', 0)
                    .attr('x2', timeScale(i))
                    .attr('y2', 30000)
                    .attr('class', 'day-line')
                    .style('opacity', 0);
                return timeScale(i) + timeLabels.length / 2;
            })
            .attr('y', calendarHeatmap.settings.label_padding / 2)
            .on('mouseenter', function(d, i) {
                ...
                calendarHeatmap.items.selectAll('.day-line')
                    ...
                    .style('opacity', function() {
                        if (selected + timeLabels.length*3 >= parseInt(this.attributes['x1'].nodeValue) && selected - 2 < parseInt(this.attributes['x1'].nodeValue)) {
                            return 1;
                        } else {
                            return 0;
                        }
                ...
                calendarHeatmap.items.selectAll('.item-block')
                    ...
                    .style('opacity', function(d) {
                        ...
                        return (selected >= start - timeLabels.length * 3  && selected <= end) ? 1 : 0.1;
                    });
            })
            .on('mouseout', function() {
                if (calendarHeatmap.in_transition) { return; }
